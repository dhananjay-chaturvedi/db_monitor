...
Options:
  --instances ID,...        Comma-separated list of RDS/Aurora instance IDs
  ...
  --include-cloud           Force AWS CloudWatch metric collection on (overrides config)
  --no-include-cloud        Force AWS CloudWatch metric collection off (overrides config)
  --help                    Show this message

Examples:
  bash run_monitor.sh my-rds-instance
  bash run_monitor.sh inst-a inst-b --interval 60
  bash run_monitor.sh --instances inst-a,inst-b --no-include-db

Collection defaults (from config.ini [monitoring]):
  collect_os_metrics    = true    Local OS metrics (CPU, memory, disk)
  collect_cloud_metrics = true    AWS CloudWatch / RDS metrics
...
Notes:
  This script runs continuously. Stop it with Ctrl-C or bash stop_monitor.sh aws.
...
_loop_pid_file() {
    # _loop_pid_file TYPE NAME → e.g. loop.1234.instance.my-rds.pid
    local type="$1" name="${2:-}"
...
# Each resource (AWS instance, SSH host, DB target, localhost OS) gets its own
...
if _poll_include_cloud_metrics; then
    for _inst in "${instances[@]}"; do
        ...
        ( poll_instance_loop "$_inst" "$inst_type" "$inst_region" "$inst_profile" "$interval" ) &
