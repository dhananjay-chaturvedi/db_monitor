...
usage() {
    cat <<EOF
monitor.sh v${VERSION} — AWS RDS / Aurora metrics, threshold alerts, daemon management
...
Commands:
  ...
  monitor       One-shot metric fetch and display for one or more instances
                  [--instance ID] [--instances saved|id1,id2] [--source os|aws|db]
  os            Collect local OS metrics (CPU, memory, disk, network)
                  [--disk PATH] [--iface IFACE]
  cloud         Collect AWS RDS / Aurora CloudWatch metrics for one instance
                  --instance ID
  ...
  instances     Register and manage RDS / Aurora instances to monitor
...
# ----------------------------------------------------------------
# AWS metric display (shared by monitor + cloud)
# ----------------------------------------------------------------
...
display_instance_aws_metrics() {
    local inst="$1" inst_type="${2:-}"

    aws_metric_fetch_deadline_begin

    echo "=== AWS CloudWatch: $inst ==="
    _fetch_and_print_metrics "$inst" "metrics are being fetched for ${inst}" \
        collect_rds_cloudwatch_metrics "$inst" "$inst_type"

    if [[ "$inst_type" == aurora-* ]]; then
        local cluster_id; cluster_id=$(rds_cluster_identifier "$inst")
        if [[ -n "$cluster_id" && "$cluster_id" != "None" ]]; then
            echo
            echo "=== Aurora cluster CloudWatch: $cluster_id ==="
            _fetch_and_print_metrics "$cluster_id" "metrics are being fetched for cluster ${cluster_id}" \
                collect_aurora_cluster_cloudwatch_metrics "$cluster_id" "$inst_type"
        fi
    fi

    if aws_collect_pi_enabled_for_instance "$inst"; then
        echo
        echo "=== Performance Insights (PI API): $inst ==="
        _fetch_and_print_metrics "$inst" "PI metrics are being fetched for ${inst}" \
            collect_rds_pi_metrics "$inst"
    fi

    if aws_collect_dbinsights_enabled_for_instance "$inst"; then
        echo
        echo "=== Database Insights (DB_PERF_INSIGHTS): $inst ==="
        _fetch_and_print_metrics "$inst" "Database Insights metrics are being fetched for ${inst}" \
            collect_rds_dbinsights_metrics "$inst"
    fi
...
cmd_monitor() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -*)
                ...
                echo "Usage: bash monitor.sh monitor [--instance ID] [--source os|aws|db]" >&2
                exit 1
                ;;
            *)
    ...
    if [[ ("$source" == "all" || "$source" == "aws") && ${#inst_list[@]} -gt 0 ]]; then
        local inst
        for inst in "${inst_list[@]}"; do
            inst="${inst//[[:space:]]/}"
            [[ -z "$inst" ]] && continue
        local inst_type="" inst_region="" inst_profile=""
        ...
        display_instance_aws_metrics "$inst" "$inst_type" || true
...
Description:
  One-shot fetch and display of AWS CloudWatch metrics for a single RDS or
  Aurora instance. Also fetches Performance Insights and Database Insights
  metrics when they are enabled for the instance in metrics_and_thresholds.ini.

Options:
  --instance, -i ID   RDS or Aurora instance identifier (required)
  --help              Show this message

Examples:
  bash monitor.sh cloud --instance prod-rds-1
  bash monitor.sh cloud -i my-aurora-cluster
EOHELP
                return 0
                ;;
            *) shift ;;
    ...
    if [[ -z "$instance" ]]; then
        echo "Usage: monitor.sh cloud --instance RDS_INSTANCE_ID  (use --help for details)" >&2
        exit 1
    fi
    local inst_type="" inst_region="" inst_profile=""
    ...
    display_instance_aws_metrics "$instance" "$inst_type"
...
cmd_instances() {
    ...
    case "$sub" in
        add)
            local name="" type="" region="" profile="default"
            while [[ $# -gt 0 ]]; do
                case "$1" in
                    --name|-n)    name="$2";    shift 2 ;;
            ...
            instances_add "$name" "$type" "$region" "$profile"
            ;;
        list)
            instances_list
            ;;
...
Description:
  Verifies that a saved RDS/Aurora instance is reachable. Checks that:
    - The instance ID exists in AWS (via describe-db-instances)
    - CloudWatch can return a CPUUtilization datapoint for it
    - The configured AWS region and profile are correct
...
Subcommands:
  add     --name ID --type TYPE [--region REGION] [--profile PROFILE]
          Register an RDS / Aurora instance for continuous monitoring
  list    Show all saved instances with type, region, and AWS profile
  test    --name ID   Verify the instance exists and CloudWatch is reachable
...
Supported types:
  ...
  sqlserver           Amazon RDS SQL Server

Examples:
  bash monitor.sh instances add --name prod-db --type aurora-mysql --region us-east-1
...
Options for list:
  ...
  --source SRC       Filter by alert source (e.g. AWS, OS, DB)
...
Examples:
  ...
  bash monitor.sh alerts list --instance prod-rds-1 --limit 20
