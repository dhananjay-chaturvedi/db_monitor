...
#
# Installs OS packages (when a package manager is available), Google Cloud CLI,
...
_install_packages() {
    ...
    case "$mgr" in
        apt)
            required=(curl wget openssh-client openssl util-linux perl libdigest-sha-perl)
            ...
            ;;
        yum|dnf)
            required=(curl wget openssh-clients openssl util-linux perl perl-Digest-SHA)
...
install_gcloud_cli() {
    if command -v gcloud &>/dev/null; then
        local ver; ver=$(gcloud --version 2>&1 | head -1)
        ok "gcloud CLI already installed: $ver"
        return
    fi

    info "gcloud CLI not found — installing Google Cloud CLI (pure bash, no Python required)"

    local arch; arch=$(uname -m)
    local url
    case "$arch" in
        x86_64)        url="https://dl.google.com/dl/cloudsdk/channels/rapid/downloads/google-cloud-cli-linux-x86_64.tar.gz" ;;
        aarch64|arm64) url="https://dl.google.com/dl/cloudsdk/channels/rapid/downloads/google-cloud-cli-linux-arm.tar.gz" ;;
        *) fail "Unsupported architecture: $arch. Install gcloud CLI manually." ;;
    esac

    local tmp_dir; tmp_dir=$(mktemp -d)
    trap 'rm -rf "$tmp_dir"' RETURN

    info "Downloading Google Cloud CLI for $arch..."
    wget -q "$url" -O "${tmp_dir}/google-cloud-cli.tar.gz"
    tar -xzf "${tmp_dir}/google-cloud-cli.tar.gz" -C "$tmp_dir"

    if [[ "$(id -u)" -eq 0 ]]; then
        local sdk_dir="/usr/local/google-cloud-sdk"
        rm -rf "$sdk_dir"
        mv "${tmp_dir}/google-cloud-sdk" "$sdk_dir"
        "${sdk_dir}/install.sh" --quiet --path-update=false
        ln -sf "${sdk_dir}/bin/gcloud" /usr/local/bin/gcloud
    elif command -v "$SUDO" &>/dev/null; then
        local sdk_dir="/usr/local/google-cloud-sdk"
        _run_as_root rm -rf "$sdk_dir"
        _run_as_root mv "${tmp_dir}/google-cloud-sdk" "$sdk_dir"
        "${sdk_dir}/install.sh" --quiet --path-update=false
        _run_as_root ln -sf "${sdk_dir}/bin/gcloud" /usr/local/bin/gcloud
    else
        local sdk_dir="${HOME}/google-cloud-sdk"
        rm -rf "$sdk_dir"
        mv "${tmp_dir}/google-cloud-sdk" "$sdk_dir"
        "${sdk_dir}/install.sh" --quiet --path-update=false
        warn "Installed gcloud to ${sdk_dir}/bin/gcloud (no root/sudo — not added to /usr/local/bin)"
        warn "Add ${sdk_dir}/bin to your PATH manually, or re-run as root to install system-wide"
    fi

    if command -v gcloud &>/dev/null; then
        ok "gcloud CLI installed: $(gcloud --version 2>&1 | head -1)"
    else
        warn "gcloud installed but not yet on PATH — open a new shell or source your profile"
    fi
}
...
check_tools() {
    local missing=()
    local tool
    for tool in bash awk grep sed ssh wget openssl xxd perl flock column timeout mktemp df install gcloud; do
...
echo "    bash ${BUNDLE_DIR}/monitor.sh cloud --instance <CLOUD_SQL_INSTANCE_ID>"
echo "    bash ${BUNDLE_DIR}/monitor.sh monitor --source all --instance <CLOUD_SQL_INSTANCE_ID>"
echo ""
echo "  Add a monitored instance:"
echo "    bash ${BUNDLE_DIR}/monitor.sh instances add --name <ID> --type <TYPE> --project <GCP_PROJECT_ID>"
echo ""
